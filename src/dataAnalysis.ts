...
export interface defaultWeightings{
...
const defaultWeights:defaultWeightings = {
    lead_id_weight: 0,
    lead_date_weight: 1,
...
export async function runAnalysis() {
    ...
    uniqueLeads.forEach((lead) => {
        ...
        lead.score = lead.score + Math.floor((365 - diffInDays) * defaultWeights.lead_date_weight * 1/365);

        //handle all other scoring:
        lead.score = lead.score + Math.floor(defaultScores.property_type_score[lead.property_type] * defaultWeights.property_type_weight * 1/4);
        lead.score = lead.score + Math.floor(defaultScores.neighbourhood_score[lead.neighbourhood] * defaultWeights.neighbourhood_weight * 1/3);
        lead.score = lead.score + Math.floor(defaultScores.requested_timeline_score[lead.requested_timeline] * defaultWeights.requested_timeline_weight * 1/3);
        lead.score = lead.score + Math.floor(defaultScores.referral_source_score[lead.referral_source] * defaultWeights.referral_source_weight * 1/3);
        lead.score = lead.score + Math.floor(defaultScores.homeowner_status_score[lead.homeowner_status] * defaultWeights.homeowner_status_weight * 1/3);
        lead.score = lead.score + Math.floor(defaultScores.customer_age_bracket_score[lead.customer_age_bracket] * defaultWeights.customer_age_bracket_weight * 1/6);
        lead.score = lead.score + Math.floor(defaultScores.expected_profit_band_score[lead.expected_profit_band] * defaultWeights.expected_profit_band_weight * 1/3);

        //handle distance scoring, closer jobs have greater scores
        lead.score = lead.score + Math.floor((50- lead.distance_to_queens_km * defaultWeights.distance_to_queens_km_weight)*1/50);
        ...
        if(lead.estimated_job_size_sqft !== null){
            ...
            lead.score = lead.score + Math.floor(lead.estimated_job_size_sqft * defaultWeights.estimated_job_size_sqft_weight * 1/4000);
        }
